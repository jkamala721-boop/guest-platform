...
class PhaseSevenIntegrationTest {
    ...
    private String register(String email)throws Exception{return TestSessionTokens.from(mvc.perform(post("/api/auth/register").contentType(MediaType.APPLICATION_JSON).content(json.writeValueAsString(Map.of("email",email,"password",PASSWORD,"passwordConfirmation",PASSWORD,"fullName","Phase Seven","phone","+254711111111")))).andExpect(status().isCreated()).andReturn());}
