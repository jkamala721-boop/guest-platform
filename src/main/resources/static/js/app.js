...
async function renderOnboarding() {
  ...
  try {
    const [onboarding, access, verification, agreement, countries] = await Promise.all([loadOnboarding(),loadOperationalAccess(),get('/api/me/verification'), get('/api/me/agreement').catch(error=>{if(error?.status===404)return null;throw error;}),loadCountries()]);
    const verificationForm = ['UNVERIFIED','REJECTED'].includes(verification.status) ? `<form id="verification-form" class="onboarding-form"><div class="form-grid"><div class="field"><label>Legal name</label><input name="legalName" required maxlength="160"></div><div class="field"><label>Verification type</label><select name="verificationType" required><option value="INDIVIDUAL">Individual</option><option value="SOLE_PROPRIETOR">Sole proprietor</option><option value="REGISTERED_BUSINESS">Registered business</option></select></div><div class="field country-field"><label for="verification-country-search">Document country</label><div class="country-combobox"><input id="verification-country-search" role="combobox" aria-autocomplete="list" aria-controls="verification-country-results" aria-expanded="false" autocomplete="off" placeholder="Search countries" required><input name="countryCode" id="verification-country" type="hidden"><div id="verification-country-results" class="country-results" role="listbox" hidden></div></div><span class="help">Select the country that issued your document.</span></div><div class="field"><label>Document type</label><select name="idType" id="verification-id-type" required><option value="NATIONAL_ID">National ID</option><option value="PASSPORT">Passport</option></select></div><div class="field"><label>ID or passport number</label><input name="idNumber" required maxlength="40" autocomplete="off"><span class="help">Used securely for verification; only a protected fingerprint and last four characters are retained.</span></div><div class="field"><label>Phone number</label><input name="phone" required maxlength="32" placeholder="+254 712 345 678"><span class="help">Use an international E.164 number. Phone country may differ from document country.</span></div></div><div class="form-actions"><button class="button" type="submit">${verification.status==='REJECTED'?'Resubmit verification':'Submit for review'}</button></div></form>` : '';
    ...
    const verificationBody = access.accountSuspended?`<div class="notice warning"><strong>Account under review.</strong><p>${escapeHtml(verification.suspensionReason||access.message||'Contact Hostvero support for assistance.')}</p></div>`:verification.status==='SUBMITTED'?'<div class="notice">Verification submitted. You can continue using Hostvero while we review your information.</div>':verification.status==='UNDER_REVIEW'?'<div class="notice">Verification under review. You can continue using Hostvero normally.</div>':verification.status==='REJECTED'?`<div class="notice warning"><strong>Verification needs an update.</strong><p>${escapeHtml(verification.rejectionReason||'Please review your details and submit again.')}</p><p>You can continue using Hostvero while correcting this information.</p></div>${verificationForm}`:verification.status==='VERIFIED'?'<p class="muted">Your identity has been verified.</p>':`${graceCopy}${verificationForm}`;
...
function bindCountryCombobox(countries) {
  const input=$('#verification-country-search'),value=$('#verification-country'),list=$('#verification-country-results'),idType=$('#verification-id-type');if(!input||!value||!list)return;let matches=[],active=-1;
  const syncDocument=()=>{const selected=countries.find(country=>country.code===value.value);const national=idType?.querySelector('option[value="NATIONAL_ID"]');if(national)national.disabled=!selected?.nationalIdSupported;if(national?.disabled&&idType.value==='NATIONAL_ID')idType.value='PASSPORT';};
  const close=()=>{list.hidden=true;input.setAttribute('aria-expanded','false');input.removeAttribute('aria-activedescendant');active=-1;};
  const select=country=>{value.value=country.code;input.value=`${country.name} (${country.code})`;input.dataset.selectedCode=country.code;syncDocument();close();};
  const draw=()=>{const query=input.value.trim().toLocaleLowerCase();matches=countries.filter(country=>!query||country.name.toLocaleLowerCase().includes(query)||country.code.toLocaleLowerCase().startsWith(query)).slice(0,50);list.innerHTML=matches.length?matches.map((country,index)=>`<button type="button" id="country-option-${index}" class="country-option" role="option" aria-selected="${index===active}" data-country-code="${country.code}"><span>${escapeHtml(country.name)}</span><small>${country.code}</small></button>`).join(''):'<p class="country-empty">No countries found</p>';list.hidden=false;input.setAttribute('aria-expanded','true');$$('.country-option',list).forEach(button=>button.addEventListener('pointerdown',event=>{event.preventDefault();select(countries.find(country=>country.code===button.dataset.countryCode));}));};
  input.addEventListener('input',()=>{value.value='';delete input.dataset.selectedCode;syncDocument();active=-1;draw();});input.addEventListener('focus',draw);input.addEventListener('blur',()=>setTimeout(close,150));input.addEventListener('keydown',event=>{if(event.key==='Escape'){close();return;}if(event.key==='ArrowDown'||event.key==='ArrowUp'){event.preventDefault();if(list.hidden)draw();active=Math.max(0,Math.min(matches.length-1,active+(event.key==='ArrowDown'?1:-1)));draw();input.setAttribute('aria-activedescendant',`country-option-${active}`);return;}if(event.key==='Enter'&&!list.hidden&&active>=0){event.preventDefault();select(matches[active]);}});syncDocument();
}
